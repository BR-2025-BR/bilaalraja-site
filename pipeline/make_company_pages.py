...
.badge{display:inline-block;font-family:var(--mono);font-size:10.5px;
  ...
  padding:4px 10px;border-radius:7px;margin-top:13px}
...
.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(148px,1fr));
  ...
  background:var(--glass);-webkit-backdrop-filter:blur(11px) saturate(165%);backdrop-filter:blur(11px) saturate(165%);
  box-shadow:0 1px 0 var(--glass-gloss) inset,0 12px 30px -20px rgba(0,0,0,.35)}
/* separators drawn as shadows, not as a background behind 1px gaps: a row that
   ...
   Use the translucent glass edge so the seams read as glass, not a table rule. */
.cell{padding:13px 15px;box-shadow:1px 0 0 var(--glass-brd),0 1px 0 var(--glass-brd)}
