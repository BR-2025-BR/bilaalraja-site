...
/* a faint ember glow that drifts ever so slightly over the static wash, so the
   ambient looks alive and the glass has something gently moving to refract.
   Transform-animated (compositor-friendly) and very slow; off for reduced motion. */
body::before{content:"";position:fixed;inset:-25%;z-index:-1;pointer-events:none;
  background:radial-gradient(38vmax 38vmax at 32% 26%,
    color-mix(in srgb,var(--ember) 10%,transparent), transparent 60%);
  will-change:transform;animation:ambientDrift 46s ease-in-out infinite alternate}
