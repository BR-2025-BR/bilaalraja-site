...
/* portfolio builder ------------------------------------------------------ */
/* minmax(0,...) on both tracks, not just min-width:0 on the items: an fr track
   floors at min-content, and relying on the item to shrink is engine-dependent
   -- Chrome contained it, Safari did not. Sizing the TRACK to allow zero is the
   part that actually holds. */
.pfgrid{display:grid;grid-template-columns:minmax(240px,340px) minmax(0,1fr);
...
.pfstat span{font-family:var(--mono);font-size:10px;letter-spacing:.09em;text-transform:uppercase;
  color:var(--ink3)}
.pfbar{display:flex;gap:9px;align-items:center;margin-bottom:5px;font-size:12.5px}
.pfbar .t{flex:0 0 118px}
.pfbar .track{flex:1 1 auto;min-width:0}
.pfbar .v{flex:0 0 92px}
...
.pfgate{display:flex;gap:9px;align-items:center;margin-bottom:5px;font-size:12.5px}
.pfgate > :first-child{flex:0 0 150px}
.pfgate .track{flex:1 1 auto;min-width:0}
.pfgate .v{flex:0 0 96px}
.pfgate > :nth-child(2):not(.track){flex:1 1 auto;min-width:0}
