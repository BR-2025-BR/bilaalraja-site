...
(function(){{
  ...
  // under tilt, which is what sells the parallax.
  const LAYERS=[{{n:70,r:[0.9,1.7],a:[.05,.11],p:6}},
                {{n:46,r:[1.5,2.6],a:[.08,.16],p:14}},
                {{n:24,r:[2.3,3.6],a:[.11,.21],p:26}}];

  function seed(){{
    dots=[];
    for(const L of LAYERS) for(let i=0;i<L.n;i++) dots.push({{
      x:Math.random(), y:Math.random(),
      r:L.r[0]+Math.random()*(L.r[1]-L.r[0]),
      a:L.a[0]+Math.random()*(L.a[1]-L.a[0]),
      p:L.p,
      vx:(Math.random()-.5)*0.000045,
      vy:(Math.random()-.5)*0.000045
    }});
  }}
  ...
  // Tilt target, driven by gyroscope on a phone or the pointer on a desktop.
  let tx=0,ty=0,cx=0,cy=0;
  addEventListener("deviceorientation",e=>{{
    if(e.gamma==null) return;
    tx=Math.max(-1,Math.min(1,e.gamma/35));          // left-right
    ty=Math.max(-1,Math.min(1,((e.beta||0)-45)/35)); // front-back, 45deg rest
  }},true);
  addEventListener("mousemove",e=>{{
    tx=(e.clientX/w-.5)*2; ty=(e.clientY/h-.5)*2;
  }},{{passive:true}});

  // iOS will not emit orientation events without explicit permission, and only
  // asks in response to a gesture. Try once on first touch, then stop.
  function askOnce(){{
  ...
  function frame(){{
    cx+=(tx-cx)*0.045; cy+=(ty-cy)*0.045;   // ease, so tilt never jerks
    x.clearRect(0,0,w,h);
    ...
    for(const d of dots){{
      ...
      x.beginPath();
      x.arc(d.x*w-cx*d.p, d.y*h-cy*d.p, d.r, 0, 6.2832);
